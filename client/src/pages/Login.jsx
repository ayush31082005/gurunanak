...
const getRedirectPathForUser = (user, locationState) => {
    if (user?.role === "admin") {
        return {
    ...
    if (locationState?.redirectTo) {
        return {
            pathname: locationState.redirectTo,
            state: locationState.checkoutState,
        };
    }
...
const Login = () => {
    ...
    useEffect(() => {
        ...
        try {
            ...
            const redirectTarget = getRedirectPathForUser(user, location.state);

            navigate(redirectTarget.pathname, {
    ...
    const handleSendOtp = async (event) => {
        ...
        try {
            ...
            setMessage(data.message || "Login OTP sent to your email");
        } catch (err) {
            const nextMessage =
            ...
            if (nextMessage.toLowerCase().includes("register first")) {
                setTimeout(() => {
                    navigate("/register");
                }, 1200);
            }
